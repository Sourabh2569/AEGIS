...
def validate_live_quotes(
    ...
    results = [
        DataQualityResult(
            ...
            passed=not rejected,
            message="Live quote records passed required read-only checks." if not rejected else "One or more live quote records failed validation.",
            affected_record_count=len(rejected),
        )
    ]
    return accepted, rejected, results
...
def freshness_snapshot(
    *,
    ...
    for record in records:
        ...
        latest_available = candidate if latest_available is None else max(latest_available, candidate)

    age_seconds = None if latest_available is None else max(0.0, (checked_at - latest_available.astimezone(timezone.utc)).total_seconds())
    status = "UNKNOWN" if age_seconds is None else ("FRESH" if age_seconds <= max_age_seconds else "STALE")
    return {
...
def derive_validation_status(results: list[DataQualityResult], critical_dataset: bool) -> ValidationStatus:
...
def quality_score(results: list[DataQualityResult]) -> float:
    if not results:
        return 100.0
    penalties = {Severity.INFO: 0.0, Severity.WARNING: 5.0, Severity.ERROR: 25.0, Severity.CRITICAL: 100.0}
    score = 100.0
