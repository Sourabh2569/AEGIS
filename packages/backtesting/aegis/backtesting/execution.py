...
def validate_order_timing(intent: OrderIntent) -> None:
    if intent.available_data_cutoff > intent.decision_time:
        raise OrderRejected(
            "DATA_UNAVAILABLE_AT_DECISION_TIME", "available_data_cutoff is after decision_time."
        )
    if intent.eligible_execution_time <= intent.decision_time:
        raise OrderRejected(
            "EXECUTION_NOT_AFTER_DECISION", "eligible_execution_time must be after decision_time."
        )
...
def reject_same_close_execution(intent: OrderIntent, execution_price_time: datetime) -> None:
    if execution_price_time <= intent.decision_time:
        raise OrderRejected(
            "SAME_CLOSE_EXECUTION_PROHIBITED",
            "Execution cannot use a price at or before decision time.",
        )
    ...
    def resolve_fill_price(
        ...
        *,
        ...
        if intent.execution_reference_type != ExecutionReferenceType.NEXT_ELIGIBLE_SESSION_OPEN:
            raise OrderRejected(
                "UNSUPPORTED_EXECUTION_REFERENCE", "Only next-session-open execution is supported."
            )
        ...
        if execution_time != intent.eligible_execution_time:
            raise OrderRejected(
                "ELIGIBLE_EXECUTION_TIME_MISMATCH", "Intent execution time does not match calendar."
            )
        reject_same_close_execution(intent, execution_time)
        price = market_data.get_open(intent.instrument_id, next_session)
        if price is None or price <= 0:
            raise OrderRejected(
                "NEXT_OPEN_PRICE_MISSING", "Next eligible session open price is missing or invalid."
            )
