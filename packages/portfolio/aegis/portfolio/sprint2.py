...
class CostModel:
    def calculate(self, gross_notional: Decimal, side: str, schedule: CostSchedule) -> CostCalculation:
        ...
        bps = schedule.brokerage_bps_buy + schedule.other_bps_buy if side == "BUY" else schedule.brokerage_bps_sell + schedule.other_bps_sell
...
class ResearchPortfolio:
    ...
    def buy(self, instrument_id: str, quantity_value: Decimal, fill_price: Decimal, total_cost: Decimal) -> None:
    ...
    def sell_t_plus_1(self, instrument_id: str, quantity_value: Decimal, fill_price: Decimal, total_cost: Decimal, settlement_date: date) -> None:
        existing_qty = self.positions.get(instrument_id, Decimal("0"))
        ...
        self.realized_pnl = money(self.realized_pnl + ((fill_price - average) * quantity_value) - total_cost)
    ...
    def settle_due(self, session: date) -> None:
        for entry in self.settlement_ledger:
            if not entry.is_settled and entry.settlement_date <= session:
                ...
                self.unsettled_receivables = money(self.unsettled_receivables - entry.cash_receivable)
