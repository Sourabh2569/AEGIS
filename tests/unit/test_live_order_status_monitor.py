...
def _open_order(repo, portfolio, *, side="BUY", broker_order_id="BROKER-1", requested_quantity=Decimal(10)) -> LiveOrder:
    now = utc_now()
    intent = LiveOrderIntent(
        ...
        side=side, proposed_quantity=requested_quantity, decision_time=now,
        available_data_cutoff=now, eligible_execution_time=now + __import__("datetime").timedelta(hours=1),
        risk_assessment_id="r1", configuration_version="v1", idempotency_key=broker_order_id,
        correlation_id="c1", approval_status=LiveApprovalDecision.APPROVED, intent_status=LiveIntentStatus.APPROVED,
    )
    repo.save_intent(intent)
    order = LiveOrder(
        live_order_intent_id=intent.live_order_intent_id, live_portfolio_id=portfolio.live_portfolio_id,
        instrument_id="AEGIS-IN-000001", status=LiveOrderStatus.SUBMITTED_TO_BROKER,
        requested_quantity=requested_quantity, idempotency_key=broker_order_id,
        broker_order_id_nullable=broker_order_id, remaining_quantity=requested_quantity,
    )
...
def test_a_complete_fill_creates_a_real_fill_and_increments_clean_fill_count(tmp_path) -> None:
    ...
    order = _open_order(repo, portfolio)
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="COMPLETE", filled_quantity=10,
        average_price="1234.50", rejection_reason=None,
    ))
...
def test_a_buy_fill_has_negative_net_cash_effect(tmp_path) -> None:
    ...
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="COMPLETE", filled_quantity=10,
        average_price="100.00", rejection_reason=None,
    ))
...
def test_a_sell_fill_has_positive_net_cash_effect_and_settles_t_plus_1(tmp_path) -> None:
    repo, adapter, monitor, portfolio = _setup(tmp_path)
    order = _open_order(repo, portfolio, side="SELL")
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="COMPLETE", filled_quantity=10,
        average_price="100.00", rejection_reason=None,
    ))
...
def test_a_sell_fill_reduces_the_real_position_and_credits_unsettled_receivables(tmp_path) -> None:
    ...
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="COMPLETE", filled_quantity=10,
        average_price="110.00", rejection_reason=None,
    ))
...
def test_a_broker_reported_oversell_becomes_an_incident_not_a_crash(tmp_path) -> None:
    ...
    reconciliation_incidents = [
        incident for incident in repo.incidents.values()
...
def test_a_broker_rejection_creates_an_incident_and_resets_clean_fill_count(tmp_path) -> None:
    ...
    adapter.script("BROKER-2", BrokerOrderStatus(
        broker_order_id="BROKER-2", status="REJECTED", filled_quantity=0,
        average_price=None, rejection_reason="Insufficient margin",
    ))
...
def test_a_partial_fill_updates_quantities_without_recording_a_fill_yet(tmp_path) -> None:
    ...
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="OPEN", filled_quantity=4,
        average_price=None, rejection_reason=None,
    ))
...
def test_a_cancelled_order_is_marked_cancelled(tmp_path) -> None:
    ...
    adapter.script("BROKER-1", BrokerOrderStatus(
        broker_order_id="BROKER-1", status="CANCELLED", filled_quantity=0,
        average_price=None, rejection_reason=None,
    ))
    updated = monitor.poll_open_orders()
...
def test_a_realistic_batch_of_open_orders_all_resolve_independently(tmp_path) -> None:
    ...
    for i in range(25):
        ...
        if i % 3 == 0:
            adapter.script(broker_order_id, BrokerOrderStatus(
                broker_order_id=broker_order_id, status="COMPLETE", filled_quantity=10,
                average_price="500.00", rejection_reason=None,
            ))
        elif i % 3 == 1:
            adapter.script(broker_order_id, BrokerOrderStatus(
                broker_order_id=broker_order_id, status="REJECTED", filled_quantity=0,
                average_price=None, rejection_reason="No liquidity",
            ))
        else:
            adapter.script(broker_order_id, BrokerOrderStatus(
                broker_order_id=broker_order_id, status="OPEN", filled_quantity=0,
                average_price=None, rejection_reason=None,
            ))
