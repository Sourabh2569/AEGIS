...
def _intent(
    portfolio: LivePortfolio, *, proposed_quantity=Decimal(10), idempotency_key="k1"
) -> LiveOrderIntent:
...
def test_active_global_kill_switch_blocks(tmp_path) -> None:
    ...
    repo.save_approval(_valid_approval(intent))
    switch = KillSwitch(
        switch_type=KillSwitchType.GLOBAL_TRADING_KILL_SWITCH,
        scope_id="GLOBAL",
        is_active=True,
        reason="halt",
    )
...
def test_inactive_kill_switch_does_not_block(tmp_path) -> None:
    ...
    repo.save_approval(_valid_approval(intent))
    switch = KillSwitch(
        switch_type=KillSwitchType.GLOBAL_TRADING_KILL_SWITCH, scope_id="GLOBAL", is_active=False
    )
    preflight = gate.check(
        intent=intent,
        portfolio=portfolio,
...
def test_pilot_capital_cap_exceeded_blocks(tmp_path) -> None:
    ...
    preflight = gate.check(
        ...
        execution_time=EXECUTION_TIME,
        entry_price=Decimal(1000),
        existing_deployed_notional=Decimal(4000),
        kill_switches=[],
    )
    assert preflight.passed is False
...
def test_pilot_capital_cap_is_not_checked_once_graduated_to_full(tmp_path) -> None:
    gate, repo = _gate(tmp_path)
    portfolio = _portfolio(pilot_capital_cap=Decimal(5000), capital_tier=LiveCapitalTier.FULL)
