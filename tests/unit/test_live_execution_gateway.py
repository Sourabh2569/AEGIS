...
class FakeOrderAdapter:
    ...
    def place_order(
        self, *, tradingsymbol, exchange, transaction_type, quantity, order_tag, correlation_id
    ):
        if self.fail_next:
...
def _setup(tmp_path):
    ...
    gateway = LiveExecutionGateway(
        repository=repo, preflight=preflight, order_adapter=adapter, audit_log=audit_log
    )
...
def _approved_intent(
    repo, portfolio, *, instrument_id="AEGIS-IN-000001", quantity=Decimal(10), idempotency_key="k1"
):
...
def test_a_realistic_multi_position_rebalance_batch_respects_the_running_capital_cap(
    tmp_path,
) -> None:
    ...
    small_cap_portfolio = LivePortfolio(
        ...
        pilot_capital_cap=Decimal(25000),
        risk_profile_version_id="v1",
        portfolio_configuration_version="v1",
        created_by="founder",
        status=LivePortfolioStatus.ACTIVE,
